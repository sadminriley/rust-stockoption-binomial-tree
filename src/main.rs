...
pub fn calculate_binomials(num_iterations: usize, price: f64, change: f64, risk: f64, strike: f64) -> f64 {
    ...
    for i in (0..n).rev() {
        for j in 0..=i {
            option[[j, i]] = (1.0 / (1.0 + r)) * (p * option[[j, i + 1]] + q * option[[j + 1, i + 1]]);
        }
    }
...
fn main() {
    // Calculate it!
    let result: f64 = calculate_binomials(1, 23.0, 1.2, 23, 5.00); // These obviously don't reflect any position
